...
func GetDateInt(timestamp int64) string {
	if timestamp <= 0 {
		return ""
	}
	tm := time.Unix(timestamp, 0)
...
func GetRemainSecond() int64 {
	nowtime := time.Now()
	loc, _ := time.LoadLocation("Local")
	tomorrow := time.Date(nowtime.Year(), nowtime.Month(), nowtime.Day()+1, 0, 0, 0, 0, loc)
	return tomorrow.Unix() - nowtime.Unix()
}
